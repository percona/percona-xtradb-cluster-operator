...
while read -ra LINE; do
	wait_for_mysql $LINE
	STATUS=$(mysql_exec "$LINE" "SHOW GLOBAL STATUS LIKE 'wsrep_%';")
	...
	echo node:$LINE:wsrep_ready:$READY:wsrep_connected:$ONLINE:wsrep_local_state_comment:$STATE:wsrep_cluster_status:$CLUSTER_STATUS:wsrep_cluster_size:$CLUSTER_SIZE
done
