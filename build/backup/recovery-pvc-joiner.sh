...
if [[ -f ${VAULT_CONFIG_FILE} ]]; then
	if check_for_version "$MYSQL_VERSION" '8.4.0'; then
		...
		PXB_VAULT_ARGS="--component-keyring-config=${VAULT_KEYRING_COMPONENT}"
	else
		PXB_VAULT_ARGS="--keyring-vault-config=${VAULT_CONFIG_FILE} --early-plugin-load=keyring_vault.so"
	fi
fi
...
xtrabackup $DEFAULTS_FILE ${XB_USE_MEMORY+--use-memory=$XB_USE_MEMORY} \
	...
	--xtrabackup-plugin-dir=/usr/lib64/xtrabackup/plugin --target-dir="$tmp" ${PXB_VAULT_ARGS}
...
echo "+ xtrabackup $DEFAULTS_FILE --defaults-group=mysqld --datadir=/datadir --move-back \
	...
	${PXB_VAULT_ARGS} --xtrabackup-plugin-dir=/usr/lib64/xtrabackup/plugin --target-dir=$tmp"
