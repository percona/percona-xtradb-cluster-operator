...
func (c *Collector) addGTIDSets(ctx context.Context, cache *HostBinlogCache, binlogs []pxc.Binlog) error {
	...
	if ok {
		...
		for i, binlog := range binlogs {
			gtidSet, ok := hostCache.Binlogs[binlog.Name]
			if !ok {
		...
		if cacheNeedsUpdate {
			if err := saveCache(ctx, c.storage, cache); err != nil {
	...
	// cache not found
	cache.Entries[c.db.GetHost()] = &BinlogCacheEntry{
...
func (c *Collector) CollectBinLogs(ctx context.Context) error {
	cache, err := loadCache(ctx, c.storage)
	if err != nil {
