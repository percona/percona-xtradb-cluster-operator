...
func (r *ReconcilePerconaXtraDBCluster) reconcilePersistentVolumes(ctx context.Context, cr *pxcv1.PerconaXtraDBCluster) (bool, error) {
	...
	if actual.Cmp(largestActual) != 0 && !cr.PVCResizeInProgress() {
		message := "PVC capacities differ across PXC pods. Update `.spec.pxc.volumeSpec.persistentVolumeClaim.resources.requests.storage` to the largest size and set spec.storageScaling.enableVolumeScaling=true"
